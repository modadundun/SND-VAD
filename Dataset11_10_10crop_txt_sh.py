    ...
    def __getitem__(self, index):
        if self.train:
            ...
            for a_i, n_i in zip(anomaly_indexs, normaly_indexs):
                ...
                categories = {'cycling': 1, 'chasing': 2, 'handcart': 3, 'fighting': 4, 'skateboarding': 5,
                ...
                for category, code in categories.items():
                    # 获取视频名称前两个数字
                    video_code = int(anomaly_data_video_name[:2])  # 取视频名称的前两个数字并转化为整数
                    if video_code == code:
                ...
                if matched_category:
                    # npy文件路径
                    npy_file_path = f'../clip_txt/txt_sh_features/{matched_category}.npy'
                ...
                # 加载正常视频特征
                normal_txt_path = f'../clip_txt/txt_sh_features/Normal.npy'
